...
function vtkPiecewiseGaussianWidget(publicAPI, model) {
  ...
  publicAPI.setDataRange = (
    dataRange
  ) => {
    model.histogram = null;
    model.dataRange = [...dataRange];
  ...
  publicAPI.getOpacityNodes = (dataRange) => {
    ...
    /*
    // without clamping
    const { position, height, width } = model.gaussians[0];
    const sampling = model.opacities.length - 1;
    for (let index = 0; index < model.opacities.length; index++) {
      const x = rangeToUse[0] + delta * index;
      let y;
      if ((index / sampling) > position + width) {
        y = 1.0 * height;
      } else {
        y = model.opacities[index];
      }
      nodes.push({ x, y, midpoint, sharpness });
    }
     */
