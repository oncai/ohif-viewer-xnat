...
const scaleRtStructContourData = (toolData, imageId) => {
  ...
  // 9 sets of simultaneous equations to choose from, choose which set to solve
  // Based on the largest component of each direction cosine.
  // This avoids NaNs or floating point errors caused by dividing by very small
  ...
  for (let pointI = 0; pointI < points.length; pointI++) {
    ...
    // General case: Solves the two chosen simultaneous equations to go from the
    // patient coordinate system to the imagePlane.
    const i =
      (c[0] - (c[1] * c[3]) / c[5]) /
      (c[2] * deltaI * (1 - (c[1] * c[4]) / (c[2] * c[5])));
    const j = (c[3] - c[4] * i * deltaI) / (c[5] * deltaJ);

    // NOTE: Add (0.5, 0.5) to the coordinate, as PCS reference frame is with
    // respect to the centre of the first pixel.
...
const scaleHandles = (toolData, imageId) => {
