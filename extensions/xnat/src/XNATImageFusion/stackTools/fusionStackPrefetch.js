...
const toolName = 'fusionStackPrefetch';
...
function prefetch(element) {
  ...
  const stack = stackData.data[1];
  if (!stack) {
    return;
  }

  // Get the fusionStackPrefetch tool data
  const stackPrefetchData = _getToolState(element);
  ...
  // If all the requests are complete, disable the fusionStackPrefetch tool
  if (
  ...
  const errorLoadingHandler = undefined;
  // const errorLoadingHandler = loadHandlerManager.getErrorLoadingHandler(
  //   element
  // );

  function failCallback(error) {
    console.log('prefetch errored: %o', error);
    if (errorLoadingHandler) {
      errorLoadingHandler(element, imageId, error, 'fusionStackPrefetch');
    }
  }
...
function enable(element) {
  ...
  const stackPrefetchDataArray = _getToolState(element);
  ...
  if (stack.preventCache === true) {
    console.warn(
      'A stack that should not be cached was given the fusionStackPrefetch'
    );
  ...
  _addToolState(element, stackPrefetchData);

  prefetch(element);

  element.removeEventListener(cornerstone.EVENTS.NEW_IMAGE, onImageUpdated);
  element.addEventListener(cornerstone.EVENTS.NEW_IMAGE, onImageUpdated);

  const promiseRemovedHandler = getPromiseRemovedHandler(element);

  cornerstone.events.removeEventListener(
    cornerstone.EVENTS.IMAGE_CACHE_PROMISE_REMOVED,
...
function _addToolState(element, stackPrefetchData) {
  ...
  toolData.data.push(stackPrefetchData);
}

function _getToolState(element) {
  const enabledElement = cornerstone.getEnabledElement(element);
  const toolStateManager = enabledElement.toolStateManager;
...
function _clearToolState(element) {
  const toolData = _getToolState(element);
...
const fusionStackPrefetch = {
  add: _addToolState,
  get: _getToolState,
  clear: _clearToolState,
  enable,
  disable,
  getConfiguration,
  setConfiguration,
...
export default fusionStackPrefetch;
