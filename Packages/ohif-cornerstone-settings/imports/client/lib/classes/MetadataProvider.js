...
export class MetadataProvider {
  ...
  updateMetadata(image) {
    ...
    if (
      ...
      (!imageMetadata.imagePlane ||
        !imageMetadata.imagePlane.imagePositionPatient[0])
    ) {
      imageMetadata.imagePlane = this.getPerFrameImagePlane(
        image,
  ...
  getPerFrameImagePlane(image, instance) {
    ...
    const frame = image.imageId.split("?frame=")[1];
    const dataSet = image.data;

    if (!dataSet.elements["x52009229"]) {
      // No SharedFunctionalGroupsSequence. Invalid.
      return;
    }
